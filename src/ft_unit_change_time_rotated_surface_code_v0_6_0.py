#!/usr/bin/env python3
"""Stim/PyMatching prospective rotated-surface-code resource audit v0.6.0.
...
The target, noise tuple, rounds-equal-distance policy, distance set, gates, and
seeds are frozen before evaluation. The primary claim is a reduction in the
minimum code distance and physical-qubit-round volume required to close a
one-sided Wilson logical-failure target. This is not a threshold theorem,
asymptotic scaling-exponent claim, formal interval certificate, compiler
result, or hardware validation.
"""
...
TITLE = "FAULT-TOLERANT RESPONSE-FIBRE ROTATED-SURFACE-CODE PROSPECTIVE RESOURCE AUDIT"
VERSION = "0.6.0"
PROSPECTIVE_SEEDS = (20290107, 20290119, 20290203)
...
def make_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--output", default="ft_unit_change_time_v0_6_0_results")
    ...
    p.add_argument("--target-logical-failure", type=float, default=0.025)
...
def main() -> int:
    ...
    if not 0.0 < args.target_logical_failure < 1.0:
        raise ValueError("target-logical-failure must lie strictly between zero and one")

    frozen_default_prospective = (
        seeds == PROSPECTIVE_SEEDS
        and distances == (3, 5, 7, 9, 11)
        and args.target_logical_failure == 0.025
        and not args.quick
    ...
    protocol = {
        ...
        "purpose": "prospectively test whether response-fibre flow lowers the minimum rotated-surface-code distance and physical-qubit-round volume required at a frozen logical-failure target",
        ...
        "source_v052_protocol_sha256": "4e3c3d3963ef4c8b29bca1aaf7568244214aa5142428f8252fb8bf811d83acd5",
        "source_v052_certificate_sha256": "f4296cce58797d301be800992830dc288537150fad00bfee0308e256e2f960f1",
        "development_target_interval_from_v052": [0.022859700317492494, 0.028340340056867457],
        "frozen_target_selection_rule": "choose the simple central value 0.025 inside the common development crossover interval before evaluating any prospective seed",
        ...
        "new_prospective_cohort_used": True,
        "frozen_default_prospective_used": frozen_default_prospective,
        "seeds": list(seeds),
        ...
        "scope": "frozen prospective fixed-reliability rotated-surface-code numerical resource audit; not a threshold theorem, asymptotic scaling-exponent claim, interval certificate, compiler result, or hardware validation",
    }
    ...
    for seed in seeds:
        ...
        print(
            f"[target={args.target_logical_failure:g} seed={seed} crossover] "
            f"pass={crossover['pass']} "
    ...
    successful_seed_fraction = sum(
        item["pass"] for item in seed_crossovers
    ) / len(seed_crossovers)
    resolved_savings = [
        item["relative_physical_qubit_round_saving"]
        for item in seed_crossovers
    ...
    prospective_cohort_gate_pass = (
        architecture_pass
        and rounds_equal_distance_gate
        and successful_seed_fraction >= args.minimum_successful_seed_fraction
    )
    claim_eligible = frozen_default_prospective and prospective_cohort_gate_pass
    report = {
        "scientific_status": (
            "PROSPECTIVE_ROTATED_SURFACE_CODE_FIXED_RELIABILITY_RESOURCE_ADVANTAGE_SUPPORTED"
            if claim_eligible
            else (
                "ROTATED_SURFACE_CODE_FIXED_RELIABILITY_RESOURCE_PREFLIGHT_SUPPORTED"
                if prospective_cohort_gate_pass
                else "PROSPECTIVE_ROTATED_SURFACE_CODE_FIXED_RELIABILITY_RESOURCE_ADVANTAGE_NOT_SUPPORTED"
            )
        ),
        "all_gates_pass": prospective_cohort_gate_pass,
        "architecture_gates_pass": architecture_pass,
        "rounds_equal_distance_gate_pass": rounds_equal_distance_gate,
        "formal_interval_arithmetic": False,
        "frozen_default_prospective_used": frozen_default_prospective,
        "new_prospective_cohort_used": True,
        "new_prospective_advantage_claimed": claim_eligible,
        "fixed_reliability_distance_reduction_claimed": claim_eligible,
        "fixed_reliability_physical_qubit_round_advantage_claimed": claim_eligible,
        "fault_tolerance_threshold_claimed": False,
        "distance_scaling_exponent_change_claimed": False,
        ...
        "crossover_seeds_passing": sum(item["pass"] for item in seed_crossovers),
        "crossover_successful_seed_fraction": successful_seed_fraction,
        "prospective_cohort_gate_pass": prospective_cohort_gate_pass,
        "frozen_target_logical_failure_probability": args.target_logical_failure,
        ...
        "next_required_step": "if the frozen fixed-reliability resource gate passes, preserve this numerical theorem boundary and next test the suppression-exponent difference on a separately frozen cohort or replace the synthetic schedule-to-fault map by a compiler-derived calibrated model",
    ...
    print("FINAL RESULT")
    print("=" * 112)
    print(json.dumps({k: v for k, v in report.items() if k not in ("cases", "seed_crossovers", "scaling_fits")}, indent=2))
    if claim_eligible:
        print("\nPASS: the frozen prospective fixed-reliability distance and physical-qubit-round advantage is supported.")
    elif prospective_cohort_gate_pass:
        print("\nPREFLIGHT PASS: the resource gates pass, but the run does not match the frozen prospective protocol.")
    else:
        print("\nFAIL-CLOSED: the frozen prospective fixed-reliability resource gate was not resolved.")
    return 0

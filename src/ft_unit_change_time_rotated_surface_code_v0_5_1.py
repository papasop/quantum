#!/usr/bin/env python3
"""Stim/PyMatching rotated-surface-code feasibility-repair audit v0.5.1.
...
This is a regression audit repairing the infeasible-initialisation stall found
by the frozen v0.5.0 prospective cohort. The v0.5.0 seeds are deliberately
reused and are therefore ineligible for a new prospective advantage claim.
It is not a threshold theorem, formal interval certificate, compiler result,
...
TITLE = "FAULT-TOLERANT RESPONSE-FIBRE ROTATED-SURFACE-CODE FEASIBILITY-REPAIR AUDIT"
VERSION = "0.5.1"
REGRESSION_SEEDS = (20280907, 20280919, 20281003)
...
def make_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--output", default="ft_unit_change_time_v0_5_1_results")
    p.add_argument("--seeds", default=",".join(map(str, REGRESSION_SEEDS)))
...
def main() -> int:
    ...
    protocol = {
        ...
        "purpose": "repair and regression-test the infeasible-initialisation stalls exposed by v0.5.0 while retaining its rotated-surface-code diagnostics",
        ...
        "source_v050_protocol_sha256": "6fcebf1cd7ad5bc7be588b395bcdf042b037cce0641298c42275197d9f53373f",
        "source_v050_certificate_sha256": "47786e2d82f0a0c627d189a1ee9560e2742ebdb304cc89c32b6d05e2a3f729bd",
        "development_seeds_excluded_from_new_claims": list(DEVELOPMENT_SEEDS_EXCLUDED),
        "regression_seeds_reused_from_v050": list(REGRESSION_SEEDS),
        "new_prospective_cohort_used": False,
        "frozen_default_regression_used": frozen_default_regression,
        "seeds": list(seeds),
        ...
        "scope": "v0.5.0 feasibility-repair regression on reused development seeds; not a new prospective advantage claim, threshold theorem, interval certificate, compiler result, or hardware validation",
    }
    ...
    crossover_diagnostic_pass = (
        architecture_pass
        and successful_seed_fraction >= args.minimum_successful_seed_fraction
    )
    regression_pass = frozen_default_regression and architecture_pass
    resolved_savings = [
    ...
    report = {
        "scientific_status": (
            "ROTATED_SURFACE_CODE_FEASIBLE_INITIALISATION_REGRESSION_REPAIRED"
            if regression_pass
            else (
                "ROTATED_SURFACE_CODE_FEASIBLE_INITIALISATION_PREFLIGHT_SUPPORTED"
                if architecture_pass
                else "ROTATED_SURFACE_CODE_FEASIBLE_INITIALISATION_REPAIR_NOT_SUPPORTED"
            )
        ),
        "all_gates_pass": architecture_pass,
        ...
        "feasible_initialisation_repair_claimed_on_reused_regression_cohort": regression_pass,
        ...
        "crossover_diagnostic_gate_pass": crossover_diagnostic_pass,
        "target_logical_failure_probability": args.target_logical_failure,
        ...
        "next_required_step": "if all twelve feasibility-repair cases accept nonzero flow and remain inside the fibre box, freeze new seeds for v0.6 before testing any prospective fixed-distance or fixed-reliability advantage; keep the reused v0.5.0 cohort development-only",
    ...
    if regression_pass:
        print("\nPASS: the v0.5.0 infeasible-initialisation stalls are repaired on the reused regression cohort.")
    elif architecture_pass:
        print("\nPREFLIGHT PASS: feasibility repair passes, but this is not the frozen default regression.")
    else:
        print("\nFAIL-CLOSED: at least one feasibility-repair or flow-architecture gate remains open.")
    return 0

#!/usr/bin/env python3
"""Stim/PyMatching fixed-reliability distance-crossover audit v0.4.0.
...
This is a prospective numerical gate-level repetition-code resource audit. It
is not a threshold theorem, surface-code result, formal interval certificate,
...
TITLE = "FAULT-TOLERANT RESPONSE-FIBRE FIXED-RELIABILITY DISTANCE-CROSSOVER AUDIT"
VERSION = "0.4.0"
PROSPECTIVE_SEEDS = (20280707, 20280719, 20280803)
...
def analyse_seed_crossover(
    ...
    if resolved:
        initial_qubits = 2 * d_initial - 1
        optimised_qubits = 2 * d_optimised - 1
...
def make_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--output", default="ft_unit_change_time_v0_4_0_results")
...
def main() -> int:
    ...
    protocol = {
        ...
        "purpose": "test whether a same-logical-X fibre flow lowers the minimum repetition-code distance and qubit-cycle volume required at a fixed logical reliability",
        ...
        "scope": "fixed-target gate-level repetition-code distance/resource audit; not a threshold theorem, surface code, interval certificate, compiler result, or hardware validation",
    }
    ...
    report = {
        "scientific_status": (
            "PROSPECTIVE_FT_FIXED_RELIABILITY_DISTANCE_AND_RESOURCE_ADVANTAGE_SUPPORTED"
            if claim_eligible
            else (
                "FT_FIXED_RELIABILITY_DISTANCE_CROSSOVER_PREFLIGHT_SUPPORTED"
                if cohort_pass
                else "FT_FIXED_RELIABILITY_DISTANCE_AND_RESOURCE_ADVANTAGE_NOT_SUPPORTED"
            )
        ),
        ...
        "next_required_step": "if the fixed-reliability distance crossover passes, transport the same preregistered resource endpoint to a rotated surface-code memory circuit; otherwise report which target distances remained unresolved",
    ...
    if claim_eligible:
        print("\nPASS: the frozen prospective fixed-reliability distance and qubit-cycle advantage is supported.")
    elif cohort_pass:
